...
class BankPertanyaanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Fix (17 Sep 2026) - ->oldest() doang gak cukup jaga urutan soal hasil import (baris yang
        // masuk dalam 1 request yang sama seringkali `created_at`-nya identik ke detik yang sama,
        // MySQL nggak jamin urutan baris yang timestamp-nya kembar). Sekarang urut pakai kolom
    ...
    public function store(Request $request)
    {
        ...
        // Fix (17 Sep 2026) - dikasih nomor urutan juga (lihat BankPertanyaanImport::collection()),
        // biar soal manual nambah di paling BAWAH daftar, konsisten sama soal hasil import.
        $validated['urutan'] = (BankPertanyaan::max('urutan') ?? 0) + 1;
   ...
   public function importExcel(Request $request)
    {
        ...
        try {
            ...
            // Jika masih 0 SAMA SEKALI (baik yang ditambah maupun ditimpa), beri tahu admin agar
            // cek format Excel. Fix (17 Sep 2026) - dulu cuma cek $import->importedCount === 0,
            // padahal sekarang import bisa "0 soal baru" tapi SUKSES (semua baris ternyata cocok
            // soal lama & ditimpa) - itu bukan kegagalan, jadi timpaCount ikut dicek di sini.
            if ($import->importedCount === 0 && $import->timpaCount === 0) {
                return response()->json([
            ...
            // Fix (17 Sep 2026) - pesan sekarang bedain jumlah soal BARU (ditambah) vs soal LAMA
            // yang ditimpa/diperbarui (lihat BankPertanyaanImport::collection()), biar Admin tahu
            // persis apa yang terjadi, bukan cuma 1 angka total.
            $pesan = "Berhasil import: {$import->importedCount} soal baru ditambahkan";
            if ($import->timpaCount > 0) {
                $pesan .= ", {$import->timpaCount} soal lama ditimpa (diperbarui)";
            }
            $pesan .= '.';
